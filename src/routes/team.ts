...
router.post('/bootstrap', asyncHandler(async (req: Request, res: Response) => {
  ...
  const me = await getMember(studioId, user.userId);
  if (me) {
...
const requireOwnerOrManager = async (req: Request, res: Response): Promise<boolean> => {
  const m = await getMember(req.studioId!, req.mediafoxUser!.userId);
...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
  const members = await getMembersByStudio(req.studioId!);
  const me = await getMember(req.studioId!, req.mediafoxUser!.userId);
  res.json({ members, my_role: me?.role ?? 'viewer' });
}));

router.get('/integration-settings', asyncHandler(async (req: Request, res: Response) => {
  if (!await requireOwnerOrManager(req, res)) return;

  const saved = await getStudioIntegrationSettingsSummary(req.studioId!);
  const effective = {
...
router.put('/integration-settings', asyncHandler(async (req: Request, res: Response) => {
  if (!await requireOwnerOrManager(req, res)) return;
  ...
  await upsertStudioIntegrationSettings(req.studioId!, req.mediafoxUser!.userId, {
    linkedin_client_id,
    linkedin_client_secret,
    linkedin_redirect_uri,
    linkedin_scopes,
...
router.post('/invite', asyncHandler(async (req: Request, res: Response) => {
  ...
  await upsertMember({ studio_id: req.studioId!, user_id: userId, email, name, role: role as never, joined_at: new Date().toISOString() });
  res.status(201).json({ ok: true, message: `${name} (${email}) added as ${role}` });
}));

router.put('/:userId/role', asyncHandler(async (req: Request, res: Response) => {
  if (!await requireOwnerOrManager(req, res)) return;
  const { role } = req.body as { role?: string };
  if (!role) { res.status(400).json({ error: 'role is required' }); return; }

  const member = await getMember(req.studioId!, req.params.userId);
  if (!member) { res.status(404).json({ error: 'Member not found' }); return; }

  await upsertMember({ ...member, role: role as never });
  res.json({ ok: true });
}));
...
router.delete('/:userId', asyncHandler(async (req: Request, res: Response) => {
  ...
  await removeMember(req.studioId!, req.params.userId);
  res.json({ ok: true });
}));
