...
router.get('/overview', (req, res) => {
  ...
  `).all(...([req.studioId, ...(from ? [from, from] : []), ...(to ? [to, to] : [])]));
    ...
    for (const row of posts) {
        ...
        for (const p of plats)
            byPlatform[p] = (byPlatform[p] ?? 0) + (row.published_count ?? 0);
    }
  ...
  `).get(...([req.studioId, ...(from ? [from] : []), ...(to ? [to] : [])]));
    res.json({
        total_published: posts.reduce((s, r) => s + (r.published_count ?? 0), 0),
...
router.get('/best-times/:accountId', (req, res) => {
    const db = (0, db_1.getDb)();
    const variants = db.prepare(`
...
router.get('/export/csv', (req, res) => {
    ...
    WHERE p.studio_id = ?
      ...
      ${to ? "AND p.published_at <= ?" : ""}
    ORDER BY p.published_at DESC
  `).all(...([req.studioId, ...(from ? [from] : []), ...(to ? [to] : [])]));
