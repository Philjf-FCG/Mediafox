...
const refreshBluesky = async (account) => {
    ...
    await (0, db_1.updateAccountTokens)(account.id, (0, crypto_1.encryptToken)(res.data.accessJwt), (0, crypto_1.encryptToken)(res.data.refreshJwt), null);
...
const refreshLinkedIn = async (account) => {
    ...
    if (!cfg.linkedinClientId || !cfg.linkedinClientSecret) {
        await (0, db_1.updateAccountStatus)(account.id, 'error');
        return;
    }
    try {
        ...
        await (0, db_1.updateAccountTokens)(account.id, (0, crypto_1.encryptToken)(res.data.access_token), res.data.refresh_token ? (0, crypto_1.encryptToken)(res.data.refresh_token) : account.refresh_token, expiresAt);
        await (0, db_1.updateAccountStatus)(account.id, 'active');
    }
    catch {
        await (0, db_1.updateAccountStatus)(account.id, 'expired');
...
const refreshExpiringTokens = async () => {
    ...
    const { rows: accounts } = await (0, db_1.getPool)().query(`SELECT * FROM accounts WHERE status='active' AND token_expires_at IS NOT NULL AND token_expires_at < $1`, [threshold]);
    for (const account of accounts) {
        try {
            ...
            // Meta tokens can be exchanged for new long-lived tokens before expiry
                ...
                const expiresAt = new Date(Date.now() + res.data.expires_in * 1000).toISOString();
                await (0, db_1.updateAccountTokens)(account.id, (0, crypto_1.encryptToken)(res.data.access_token), null, expiresAt);
                await (0, db_1.updateAccountStatus)(account.id, 'active');
            }
        }
        catch (err) {
