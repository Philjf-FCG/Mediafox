...
export default function Login({ onAuthenticated }: Props) {
  ...
  const handleDevLogin = async () => {
    ...
    try {
      // Dev bypass: just call /me which returns the dev user when auth is disabled
      await onAuthenticated();
    } catch {
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        {googleClientId ? (
          scriptLoaded ? (
            <>
              <p style={{ color: '#94a3b8', fontSize: 14 }}>Sign in with Google to continue.</p>
              <div ref={btnRef} style={styles.btnWrap} />
            </>
          ) : (
            <p style={{ color: '#64748b', fontSize: 14 }}>Loading sign-in...</p>
          )
        ) : (
          <>
            <p style={{ color: '#94a3b8', fontSize: 14 }}>Google login is not configured. Use dev bypass for local testing.</p>
